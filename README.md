...
인증은 **Codex 로그인 하나**입니다 — 설정(⚙)이나 첫 화면의 버튼을 누르면 `codex login`(ChatGPT 계정 OAuth)이 브라우저에서 열립니다. **API 키는 필요 없습니다.** 페이지를 구성할 때 앱은 그 로그인 세션을 그대로 쓰는 Codex CLI(`codex exec --output-schema …`)로 구조화된 결과를 받아옵니다. 로그인하지 않아도 앱은 오프라인 플래너로 완전히 동작하며, 설정에 그 상태가 그대로 표시됩니다.

Codex CLI가 없다면 `npm i -g @openai/codex` 후 `codex login`.
