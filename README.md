...
- **Generated View — 소스별 나열이 아니라 합성된 한 페이지** — YouTube·뉴스 피드·Hacker News·Lobsters·Reddit에서 모은 콘텐츠를 **가로질러** 구성합니다. 페이지는 여러 소스가 함께 말하는 바를 요약한 **합성 브리핑**(문장마다 인용 근거 표시)으로 열고, 같은 주제를 다룬 서로 다른 소스를 **주제 묶음**으로 모으며, 목록형 블록도 소스를 교차 배치해 한 매체로만 채워지지 않습니다. LLM 플래너(`claude-opus-5`, 구조화 출력)가 **큐레이션된 컴포넌트 카탈로그**로만 구성하고, 렌더러가 최종 검증·수리를 소유합니다. 자격 증명이 없으면 오프라인 휴리스틱 플래너가 같은 합성 규칙으로 동작합니다.
